...
class SyncManager(private val authMgr: AuthManager, private val featMgr: FeatureManager) {
  ...
  private fun downloadDefinitions() = Completable.create { emitter ->
    try {
      progress.onNext(0f)
      val defs = eosGraphQL.queryDefinitions(featMgr.createCollectionEnabled.blockingFirst()).blockingGet()
      ...
      val vehicles = (defs.data?.vehicles ?: listOf()).map { veh ->
        Vehicle(
          id = 0,
          vehicleLicensePlate = veh.licensePlate,
          countyId = veh.county.id
        )
      }

      progress.onNext(1f)
      Database.county.rxInsert(counties!!).blockingAwait()
      Database.uat.rxInsert(uats!!).blockingAwait()
      Database.loc.rxInsert(locs!!).blockingAwait()
      Database.artery.rxInsert(arteries!!).blockingAwait()
      Database.recLabel.rxInsert(recLabels!!).blockingAwait()

      if(vehicles.isNotEmpty()) {
        runBlocking { Database.vehicle.insert(vehicles) }
      }
      ...
      do {
        ...
        val recipients = results?.data?.recipients?.items?.map { r ->
          Recipient(
            ...
            lifecycle = r.lifecycle.toString(),
            comments = r.comments,
            size = r.size,
        ...
        Database.recipient.rxInsert(recipients!!).blockingAwait()
  ...
  private fun downloadGroups() = Completable.create { emitter ->
    try {
      ...
      do {
        ...
        Database.group.rxInsert(groups!!).blockingAwait()
  ...
  private fun downloadRecipientTags() = Completable.create { emitter ->
    try {
      ...
      do {
        ...
        Database.recipientTags.rxInsert(recipientTags!!).blockingAwait()
        Log.d(tag, "Inserted into DB")
        ++pageNumber
  ...
  private fun uploadCollections() = Completable.create { emitter ->
    try {
      val collectionsToCreate = Database.collection.getPendingUpload().blockingGet()
      progress.onNext(0f)
      val total = collectionsToCreate.size + 1f
      var ctr = 0f
      for (collection in collectionsToCreate) {
        val result = eosGraphQL.createCollection(collection).blockingGet()
        if(result.data != null) {
          progress.onNext(ctr / total)
          Database.collection.markUploaded(collection.id)
          ++ctr
        } else {
          break
        }
  ...
  private fun clearDatabase(): Completable =
    Database.recLabel.rxDeleteAll()
      .andThen(Database.recipient.rxDeleteAll())
      .andThen(Database.group.deleteAll())
      .andThen(Database.artery.rxDeleteAll())
      .andThen(Database.loc.rxDeleteAll())
      .andThen(Database.uat.rxDeleteAll())
      .andThen(Database.county.rxDeleteAll())
      .subscribeOn(Schedulers.io())
    ...
    if (authMgr.session.isValid.blockingFirst()) {
      ...
      when (Preferences.smState.value) {
        ...
        SyncState.UPDATE_TAGS -> {
          ...
          pendingSub = pendingCompletable
            ?.subscribe({
              val nextState = if(featMgr.createCollectionEnabled.blockingFirst()) {
                SyncState.UPLOAD_COLLECTIONS
              } else {
                SyncState.SYNC_RECIPIENTS
              }
              Preferences.smState.onNext(nextState)
        ...
        SyncState.UPLOAD_COLLECTIONS -> {
          pendingCompletable = uploadCollections()
          pendingSub = pendingCompletable
            ?.doFinally { finalizeCompletable() }
            ?.subscribe({
